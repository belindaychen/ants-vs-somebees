...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while pos.entrance is not None:
            if step_count > self.max_range:
                return None
            if not pos.is_hive:
                if len(pos.bees) > 0 and step_count >= self.min_range:
...
class TankAnt(ContainerAnt):
    """TankAnt is a container ant that protects an ant in its place and also deals 1 damage to all bees in its place each turn."""
    ...
    def action(self, gamestate):
        ...
        while spot:
            ...
            if ant and isinstance(ant, ContainerAnt):
                if ant.ant_contained and ant.ant_contained not in self.buffed_ants:
                    ant.ant_contained.damage *= 2
                    self.buffed_ants.append(ant.ant_contained)
            spot = spot.exit
